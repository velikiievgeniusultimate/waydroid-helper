...
import gi
from gi.repository import Adw, Gdk, Gtk, GLib, Pango, GObject, GdkPixbuf
...
class ContextMenuManager:
    ...
    def __init__(self, parent_window: "TransparentWindow"):
        ...
        self._profile_hotkey: "KeyCombination | None" = None
    ...
    def _ensure_profile_manager_css(self) -> None:
        ...
        button.profile-avatar-button-active {
            border-color: rgba(53, 132, 228, 1.0);
        }
        picture.profile-avatar-image {
            border-radius: 999px;
        }
        """
    ...
    def _load_profile_hotkey(self) -> "KeyCombination | None":
        if not hasattr(self.parent_window, "key_registry"):
            return None
    ...
    def reload_profile_hotkey(self) -> None:
        self._profile_hotkey = self._load_profile_hotkey()
    ...
    def handle_profile_hotkey_press(
        ...
        if self._profile_hotkey is None:
            self._profile_hotkey = self._load_profile_hotkey()
        if self._profile_hotkey is None or len(self._profile_hotkey) == 0:
            return False
    ...
    def _get_profile_avatars_dir(self) -> Path:
        avatars_dir = Path(self._get_profiles_dir()) / "avatars"
        avatars_dir.mkdir(parents=True, exist_ok=True)
        return avatars_dir

    def _profile_avatar_path(self, profile_name: str) -> Path:
        return self._get_profile_avatars_dir() / f"{profile_name}.png"

    def _save_profile_avatar(self, profile_name: str, source_path: str) -> bool:
        try:
            ...
            max_size = 256
            scale = max(max_size / src_w, max_size / src_h)
            ...
            src_x = max(0, (scaled_w - max_size) // 2)
            src_y = max(0, (scaled_h - max_size) // 2)
            scaled.copy_area(src_x, src_y, max_size, max_size, canvas, 0, 0)

            avatar_path = self._profile_avatar_path(profile_name)
            canvas.savev(str(avatar_path), "png", [], [])
            return True
        except Exception as exc:
            logger.error("Failed to save profile avatar: %s", exc)
            return False

    def _change_profile_image(self, profile_name: str, refresh_tiles: Callable[[], None]) -> None:
        normalized = self._normalize_profile_name(profile_name)
        if not normalized:
            return

        transient_for = self._profile_manager_window or self.parent_window
        chooser = Gtk.FileChooserNative.new(
            _("Select profile image"),
            transient_for,
        ...
        for pattern in ["*.png", "*.jpg", "*.jpeg", "*.webp", "*.bmp", "*.gif", "*.tiff", "*.svg"]:
            image_filter.add_pattern(pattern)
        chooser.set_filter(image_filter)

        def on_response(dialog: Gtk.FileChooserNative, response: int) -> None:
            if response == Gtk.ResponseType.ACCEPT:
                file = dialog.get_file()
                if file:
                    path = file.get_path()
                    if path and self._save_profile_avatar(normalized, path):
                        self.parent_window.show_notification(_("Profile image updated"))
                        refresh_tiles()
                    else:
                        self.parent_window.show_notification(_("Failed to update profile image"))
            dialog.destroy()
    ...
    def _apply_layout_data(
        ...
        if hasattr(self.parent_window, "set_all_widgets_mapping_mode"):
            self.parent_window.set_all_widgets_mapping_mode(
                self.parent_window.current_mode == self.parent_window.MAPPING_MODE
            )
    ...
    def _rename_profile(self, old_name: str, new_name: str) -> None:
        ...
        old_avatar = self._profile_avatar_path(normalized_old)
        new_avatar = self._profile_avatar_path(normalized_new)
    ...
    def _delete_profile(self, profile_name: str) -> None:
        ...
        self._profile_avatar_path(normalized).unlink(missing_ok=True)
    ...
    def _show_profile_context_menu(
        ...
        request_change_image: Callable[[str], None],
    ) -> None:
        popover = Gtk.Popover()
        ...
        change_image_button = Gtk.Button(label=_("Change image"))
        change_image_button.add_css_class("flat")
        change_image_button.connect(
            "clicked",
            lambda _btn: (
                request_change_image(profile_name),
                popover.popdown(),
            ),
        )
        menu_box.append(change_image_button)
    ...
    def _create_profile_tile(
        ...
        if has_avatar:
            avatar_pixbuf = GdkPixbuf.Pixbuf.new_from_file(str(avatar_path))
            avatar_area = Gtk.DrawingArea()
            avatar_area.set_size_request(76, 76)
            avatar_area.add_css_class("profile-avatar-image")
            is_active = profile_name == self._current_profile

            def draw_avatar(_area: Gtk.DrawingArea, cr, width: int, height: int, pixbuf: GdkPixbuf.Pixbuf) -> None:
                src_w = pixbuf.get_width()
                src_h = pixbuf.get_height()
                if src_w <= 0 or src_h <= 0:
                    return
                ...
                ring_width = 3.0
                outer_radius = (min(width, height) / 2.0) - 1.0
                image_radius = max(1.0, outer_radius - ring_width)

                scale = max((image_radius * 2.0) / src_w, (image_radius * 2.0) / src_h)
                draw_w = src_w * scale
                draw_h = src_h * scale
                ...
                cr.save()
                cr.arc(width / 2.0, height / 2.0, image_radius, 0, 6.283185307179586)
                cr.clip()
                ...
                cr.arc(width / 2.0, height / 2.0, outer_radius - (ring_width / 2.0), 0, 6.283185307179586)
                cr.stroke()

            avatar_area.set_draw_func(draw_avatar, avatar_pixbuf)
            icon_button.set_child(avatar_area)
        else:
        ...
        right_click.connect(
            ...
            lambda _gesture, _n_press, _x, _y: self._show_profile_context_menu(
                tile, profile_name, refresh_tiles, request_rename, request_change_image
            ),
        )
    ...
    def _build_profile_manager_content(self, widget_factory: "WidgetFactory") -> Gtk.Box:
        ...
        def build_inline_editor(
            ...
            editor.set_width_request(88)

            entry_holder = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
            entry_holder.set_size_request(88, 88)
            entry_holder.set_valign(Gtk.Align.CENTER)
            entry_holder.set_halign(Gtk.Align.FILL)
            ...
            entry.set_margin_top(28)
            entry.set_margin_bottom(28)
            entry_holder.append(entry)
            editor.append(entry_holder)

            action_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            action_row.set_width_request(88)
            ...
            accept_button.set_halign(Gtk.Align.END)
            accept_button.connect("clicked", lambda _btn: on_confirm(entry.get_text()))
            action_row.append(accept_button)
        ...
        def request_change_image(profile_name: str) -> None:
            self._change_profile_image(profile_name, refresh_tiles)
        ...
        def refresh_tiles() -> None:
            ...
            for profile_name in self._list_profiles():
                ...
                if editor_state["mode"] == "rename" and editor_state["target"] == profile_name:
                    ...
                    first_child = tile.get_first_child()
                    if first_child is not None:
                        tile.remove(first_child)
                    tile.prepend(editor)
            ...
            if editor_state["mode"] == "add":
                ...
                first_child = add_tile.get_first_child()
                if first_child is not None:
                    add_tile.remove(first_child)
                add_tile.prepend(editor)
