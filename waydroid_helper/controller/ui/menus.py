...
class ContextMenuManager:
    ...
    def _save_profile_avatar(self, profile_name: str, source_path: str) -> bool:
        try:
            ...
            # Apply circular alpha mask so corners are truly clipped.
            pixels = canvas.get_pixels()
            rowstride = canvas.get_rowstride()
            n_channels = canvas.get_n_channels()
            center = max_size / 2.0
            radius = center - 1.0
            ...
            for y in range(max_size):
                for x in range(max_size):
                    ...
                    if (dx * dx + dy * dy) > radius_sq:
                        alpha_index = y * rowstride + x * n_channels + 3
    ...
    def _create_profile_tile(
        ...
        if has_avatar:
            ...
            avatar.set_size_request(76, 76)
            avatar.add_css_class("profile-avatar-image")
            icon_button.set_child(avatar)
        else:
            if profile_name == self._current_profile:
