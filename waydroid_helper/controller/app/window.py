...
class CircleOverlay(Gtk.DrawingArea):
    ...
    def _draw_circle(self, widget, cr, width, height, user_data):
        ...
        if (math_center_x, math_center_y) != (center_x, center_y):
            ellipse_crosshair = 6
            cr.set_source_rgba(1.0, 0.7, 0.2, 0.9)
            cr.set_line_width(2)
            cr.move_to(math_center_x - ellipse_crosshair, math_center_y)
            cr.line_to(math_center_x + ellipse_crosshair, math_center_y)
...
class RightClickToWalkOverlay(Gtk.DrawingArea):
    ...
    def _draw_overlay(self, widget, cr, width, height, user_data):
        ...
        if not self.mapping_mode and not is_calibrating and not tuning_active:
...
class TransparentWindow(Adw.Window):
    ...
    def __init__(self, app, display_name: str, host_display_name: str | None = None):
        ...
        self._cage_fullscreen = False
        self._cage_windowed_size: tuple[int, int] | None = None
    ...
    def _get_monitor_size(self) -> tuple[int, int] | None:
        display = self.get_display()
        if display is None:
            return None
        surface = self.get_surface()
    ...
    def _toggle_cage_resize(self) -> None:
        if not self._cage_fullscreen:
            if self._cage_windowed_size is None:
                self._cage_windowed_size = (
            ...
            if monitor_size is None:
                return
            self.unmaximize()
            self.set_default_size(monitor_size[0], monitor_size[1])
            self.present()
            self._cage_fullscreen = True
        else:
            self.unmaximize()
            if self._cage_windowed_size is not None:
                self.set_default_size(
                    self._cage_windowed_size[0], self._cage_windowed_size[1]
                )
            self.present()
            self._cage_fullscreen = False
    ...
    def update_layout(self) -> None:
        ...
        is_fullscreen = self.is_fullscreen() or self._cage_fullscreen
        if is_fullscreen:
        ...
        self._is_fullscreen_mode = is_fullscreen
        show_bars = is_fullscreen and (offset_x > 0 or offset_y > 0)
